 ...
 * @param {string} systemPrompt - Optional custom system prompt from project
 * @returns {Array} Formatted messages for OpenRouter
 */
function formatMessagesForOpenRouter(messages, uploadsDir, enableImageGeneration = false, systemPrompt = null) {
  ...
  if (systemPrompt) {
    formattedMessages.push({
      ...
      content: addCacheControl([{
        type: 'text',
        text: systemPrompt
      }], true) // Always cache system messages
    })
  }

  const userMessages = messages.map((msg, index) => {
  ...
  // Combine system message (if any) with user messages
  return [...formattedMessages, ...userMessages]
}
